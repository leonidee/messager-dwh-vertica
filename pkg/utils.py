...
from pkg.errors import DotEnvError, VerticaError, S3Error
...
class Connector:
    def __init__(self, type: Literal["vertica-dwh", "s3"]) -> None:
        self.type = type
        ...
        if self.type == "s3":
            try:
                holder = dict(
                    ...
                    endpoint=getenv("AWS_ENPOINT_URL"),
                )
                logger.info(f"All variables loaded for `{self.type}` connection.")
            except Exception:
                logger.exception(
    ...
    def connect(self) -> Union[Engine, Session]:
        ...
        if self.type == "vertica-dwh":
            ...
            conn_obj = create_engine(
                url=f"vertica+vertica_python://{holder['user']}:{holder['password']}@{holder['host']}:{holder['port']}/{holder['database']}"
            )
            try:
